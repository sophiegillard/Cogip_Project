...
class companies
{
    ...
    {
        $db = (new dbConnection())->connexion();
        $query = $db->prepare('SELECT companies.name,tva,country,companies.created_at, contacts.name AS contactName, contacts.phone AS phoneNumber FROM `companies` LEFT JOIN contacts ON contacts.company_id = companies.id WHERE companies.id = :id;');
        $query->execute(array(
            'id' => $id
        ));
        $db = null;
